...
const sellfun = async (client) => {
    ...
    console.log(getNowFormatDate(), `正在卖出中... 卖${quantitys}个SOL`);
    console.log(getNowFormatDate(), "***************************最新买入价:", last_buy_price);
    let sellprice=last_buy_price*1.0021;//将卖出价提升为买入价的1.0025倍  抵消价差亏损及买卖手续费 以此减少磨损
    console.log(getNowFormatDate(), "***************************挂单卖出价:", sellprice);
    if(lastPriceask>=sellprice || sellcount>3000)
    {
...
const buyfun = async (client) => {
    ...
    if (orderResultBid?.status == "Filled" && orderResultBid?.side == "Bid") {
        ...
        last_buy_price=orderResultBid.price;//最新买入价格
        init(client);
    } else {
...
(async () => {
    const apisecret = "wsDQ5JwP6e/g4q6j9/nUfV46jdg1Gp1NoxTJEkrJU2A=";
    const apikey = "baw6nqfCIZSYpT0zTGEEPwpGpq1n7Gu6XxWOmNBpJOs=";
